...
def flag_sd_pandas(Df: pd.Dataframe, STD: float = 1.96, outlier_vars: list = [], **kwargs) -> pd.DataFrame:
    ...
    Args:
        ...
        STD (float): standard deviations out to flag as outliers (two sided)
                    (default: 1.96)
        outlier_vars (list of str or ints): list of variables to use to calculate
    ...
    lower_bound = mean_vals - STD*sd_vales
    upper_bound = mean_vals + STD*sd_vales
